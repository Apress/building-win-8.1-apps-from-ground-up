...
namespace BW8AGU.Samples.CSharp
{
    ...
    /// </summary>
    public sealed partial class LiveTilesSample : Page
    {

        private NavigationHelper navigationHelper;
        private ObservableDictionary defaultViewModel = new ObservableDictionary();
        private ObservableCollection<string> templatesName = new ObservableCollection<string>();
        private TileTemplateType selectedTemplate;
        ...
        {
            foreach (var item in Enum.GetValues(typeof(TileTemplateType)))
            {
        ...
        private void ShowTestNotification(object sender, RoutedEventArgs e)
        {
            XmlDocument document = new XmlDocument();
            
            document.LoadXml(xmlResultNameTextBox.Text);
            var updater = TileUpdateManager.CreateTileUpdaterForApplication();
            //updater.EnableNotificationQueue((bool)defaultViewModel["UseQueue"]);
            updater.Clear();
            updater.Update(new TileNotification(document));
            //updater.AddToSchedule(new ScheduledTileNotification(document, DateTimeOffset.Now.AddSeconds(20)));
