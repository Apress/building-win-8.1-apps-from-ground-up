...
''' </summary>
Public NotInheritable Class CompassSample
    Inherits Page
...
#End Region
    ...
    Private Sub OnGetDataButtonClick(sender As Object, e As RoutedEventArgs)
        Dim compassIstance = Compass.GetDefault()
        If compassIstance Is Nothing Then
            Me.DefaultViewModel("CompassData") = New With {
                Key .MagneticNorth = "Compass not found",
                Key .TrueNorth = "Compass not found",
                Key .Accuracy = "Compass not found"
            }
        Else
            Dim reading = compassIstance.GetCurrentReading()
            AddHandler compassIstance.ReadingChanged, AddressOf OnCompassReadingChanged
            SetCompassReading(reading)
        End If
    End Sub

    Private Sub OnCompassReadingChanged(sender As Compass, args As CompassReadingChangedEventArgs)
        SetCompassReading(args.Reading)
    End Sub

    Private Sub SetCompassReading(reading As CompassReading)
        Me.DefaultViewModel("CompassData") = New With {
